...
(function(window, Phaser) {
    ...
    Phaser.Plugin.IsometricControl.prototype.createLayer = function(layer_name) {
        ...
        for (var x = 0; x < this.width; x++)
        {
            ...
            {
                row.push(new Phaser.Tile(this.map.layers[layer], this.map.layers[layer].data[tile_position], (this.width-1)*(this.tileWidth/2) - (x*(this.tileWidth/2)) + y*(this.tileWidth/2), (x+y)*this.tileHeight/2, this.tileWidth, this.tileHeight));
                tile_position++;
            }
            output.push(row);
        }
